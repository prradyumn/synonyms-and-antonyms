...
\| far | \`far\_sky\` | 3840×1080 | 0.20 | opaque, covers the frame |

\| mid | \`mid\_canopy\` | 3840×1080 | 0.50 | treeline |

\| near | \`near\_leaves\` | 3840×169 | 1.40 | top-edge leaves |

\| near | \`near\_grass\` | 3840×236 | 1.40 | bottom-edge grass |
...
\| 1 | \`act\_gorge\_near\` | the approach | path climbing out of the clearing; the ravine's near rim; **\*\*the broken bridge already visible ahead\*\***, small, across the gap |

\| 2 | \`act\_gorge\_span\` | the hurdle | the bridge filling the frame, the two-plank gap dead centre |

\| 3 | \`act\_gorge\_far\` | the far side | the far rim, the path continuing into trees — where the game screen takes over |
...
\| 4 | \`mid\_gorge\` | 5760×1080 | 0.42 | the inside of the ravine — walls receding, a river far below, mist. Seen **\*\*through\*\*** the gap and under the bridge. |

\`mid\_gorge\` is 3× a frame wide because it sits behind all three action plates and
...
breaks the effect. \`mid\_gorge\` sits behind and travels at 0.42 against the deck's
...
Attach **\*\*\`assets/source/px\_act\_bridge.png\`\*\*** as the style reference every time, and

**\*\*\`assets/source/px\_act\_clearing.png\`\*\*** as well for plate 1.
...
**## 1 · \`act\_gorge\_near\` — the approach**
...
**## 3 · \`act\_gorge\_far\` — the far side**
...
**## 4 · \`mid\_gorge\` — what you see down the hole**
...
**\*\*5 · \`prop\_plank\`\*\*** — one loose plank on its own transparent canvas, \~300×60. The
...
      deck on plate 2, and the top 30% of \`mid\_gorge\` must all be magenta. This is
...
      fight \`near\_leaves\` (169px) or \`near\_grass\` (236px).
...
\- add \`mid\_gorge\` as a positioned slow layer behind the action plane, and verify the
...
\- put a join overlay on each segment join, as \`join\_trunk\` already does for the
